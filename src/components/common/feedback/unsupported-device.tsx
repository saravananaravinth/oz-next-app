// oz-next-app/src/components/common/feedback/unsupported-device.tsx
...
export type UnsupportedDeviceProps = Readonly<{
  title?: string;
  description?: string;
  backHref?: string;
  backLabel?: string;
...
const DEFAULT_TITLE = "This screen needs a larger workspace";
const DEFAULT_DESCRIPTION =
  "Use a tablet, laptop, or desktop display to access this ERP screen safely.";
const DEFAULT_BACK_LABEL = "Back to dashboard";
const DEFAULT_SUPPORTING_TEXT =
  "This protects high-density ERP tables, forms, and operational workflows from accidental input.";
...
export function UnsupportedDevice({
  ...
  backHref,
  backLabel,
}: UnsupportedDeviceProps): ReactElement {
  const reactId = useId();
  const titleId = `${reactId}-unsupported-device-title`;
  const descriptionId = `${reactId}-unsupported-device-description`;
  const supportingTextId = `${reactId}-unsupported-device-supporting-text`;
    ...
    >
      <Card className="w-full max-w-xl text-center">
        <CardHeader className="items-center gap-5 px-6 pt-8 sm:px-8 sm:pt-10">
          <div className="flex size-14 items-center justify-center rounded-2xl border border-border/70 bg-warning/10 text-warning-foreground shadow-xs shadow-foreground/5 dark:text-warning">
