...
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        [
          "group/input-group relative flex h-11 w-full min-w-0 items-center rounded-2xl border border-input bg-background shadow-xs outline-none",
...
const inputGroupAddonVariants = cva(
  "flex h-auto cursor-text items-center justify-center gap-2 py-1.5 text-body-sm text-muted-readable select-none group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-md [&>svg:not([class*='size-'])]:size-4",
  {
    variants: {
      align: {
        "inline-start":
          "order-first ps-3 has-[>button]:ms-[-0.35rem] has-[>kbd]:ms-[-0.15rem]",
        "inline-end":
          "order-last pe-3 has-[>button]:me-[-0.35rem] has-[>kbd]:me-[-0.15rem]",
        "block-start":
