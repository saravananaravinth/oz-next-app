...
import { readPurchaseDocumentResponse } from "@/features/wallet/server/wallet.server";
...
type RouteContext = Readonly<{
  params: Promise<Readonly<{ invoiceId: string; source: string }>>;
}>;
...
export async function GET(
  ...
  try {
    const upstream = await readPurchaseDocumentResponse({
      invoiceId: params.data.invoiceId,
      source: params.data.source,
      cycleId: query.data.cycleId,
    ...
    if (
      contentType.split(";")[0]?.trim() !== CT.PDF ||
      upstream.body === null ||
