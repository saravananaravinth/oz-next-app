// oz-next-app/src/features/engagement/location-requests/ui/location-request-shell.tsx
...
export type PublicLocationShellProps = Readonly<{
  children: ReactNode;
...
type PublicLocationBrandMarkProps = Readonly<{
  className?: string;
  iconClassName?: string;
}>;

function PublicLocationBrandMark({
  className,
  iconClassName,
}: PublicLocationBrandMarkProps): ReactElement {
  return (
    <span
      aria-hidden="true"
      className={cn(
...
export function PublicLocationShell({
  children,
  footerActions,
  mainLabelledBy,
  mainClassName,
  mainRef,
}: PublicLocationShellProps): ReactElement {
  ...
  return (
    <div className="relative isolate flex min-h-svh min-w-0 flex-col bg-background text-foreground">
      ...
      <aside
        aria-label="Location privacy and safety"
      ...
      >
        <div className="mx-auto flex max-w-3xl items-start justify-center gap-2.5">
          ...
          <p className="max-w-2xl text-left text-caption leading-relaxed text-muted-readable text-pretty sm:text-center">
            ...
            Location is requested only after browser permission. Ozotec does not
            ask for OTPs, payments, bank details, or identity documents on this
            page.
          </p>
        </div>
      </aside>
